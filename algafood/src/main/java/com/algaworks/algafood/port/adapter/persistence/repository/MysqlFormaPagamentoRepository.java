...
import com.algaworks.algafood.port.adapter.service.FormaPagmentoTranslator;
...
public class MysqlFormaPagamentoRepository implements FormaPagamentoRepository {
    ...
    @Override
    public Optional<FormaPagamento> buscar(String id) {
    ...
    public void atualizar(FormaPagamento formaPagamento) {
        manager.createQuery("UPDATE FormaPagamentoModel fp SET fp.descricao = ?1  WHERE fp.id = ?2")
                ...
                .setParameter(2,  UUID.fromString(formaPagamento.getId()))
                .executeUpdate();
    }
    ...
    public void remover(FormaPagamento formaPagamento) {
        manager.createQuery("DELETE FROM FormaPagamentoModel fp WHERE fp.id = ?1")
                .setParameter(1, UUID.fromString(formaPagamento.getId()))
                .executeUpdate();
    }
    ...
    @Override
    public boolean existeFormaPagamentoComDescricaoComIdDiferente(String descricao, String id) {
        return manager.createQuery(
                        "select case when count(fp)> 0 then true else false " +
