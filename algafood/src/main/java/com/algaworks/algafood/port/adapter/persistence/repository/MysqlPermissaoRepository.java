...
import com.algaworks.algafood.port.adapter.service.PermissaoTranslator;
...
public class MysqlPermissaoRepository implements PermissaoRepository {
    ...
    @Override
    public Optional<Permissao> buscar(String id) {
    ...
    public void atualizar(Permissao permissao) {
        manager.createQuery("UPDATE PermissaoModel p SET p.nome = ?1, p.descricao = ?2  WHERE p.id = ?3")
                ...
                .setParameter(3, UUID.fromString(permissao.getId()))
                .executeUpdate();
    }
    ...
    public void remover(Permissao permissao) {
        manager.createQuery("DELETE FROM PermissaoModel p WHERE p.id = ?1")
                .setParameter(1, UUID.fromString(permissao.getId()))
                .executeUpdate();
    }
    ...
    @Override
    public boolean existePermissaoComNomeComIdDiferente(String nome, String id) {
        return manager.createQuery(
                        "select case when count(p)> 0 then true else false " +
